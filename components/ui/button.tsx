...
function Button({
  ...
  children,
  ...props
}: ButtonPrimitive.Props & VariantProps<typeof buttonVariants> & { neon?: boolean }) {
  return (
    <ButtonPrimitive
      data-slot="button"
      ...
      {...props}
    >
      <span className={cn("absolute h-[2px] opacity-0 group-hover/button:opacity-100 transition-all duration-500 ease-in-out inset-x-0 top-0 bg-gradient-to-r w-3/4 mx-auto from-transparent via-primary to-transparent hidden pointer-events-none z-10", neon && "block")} />
      {children}
      <span className={cn("absolute opacity-0 group-hover/button:opacity-100 transition-all duration-500 ease-in-out inset-x-0 h-[2px] bottom-0 bg-gradient-to-r w-3/4 mx-auto from-transparent via-primary to-transparent hidden pointer-events-none z-10", neon && "block")} />
    </ButtonPrimitive>
  )
}
