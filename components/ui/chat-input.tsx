...
function ChatInputSubmit({
    ...
    if (loading && onStop) {
        return (
            <button
                ...
                onClick={onStop}
                className={cn(
                    "shrink-0 rounded-full p-1.5 h-fit border border-input bg-background hover:bg-muted transition-colors",
                    className,
                )}
                {...props}
            >
    ...
    return (
        <button
            type="button"
            className={cn(
                "shrink-0 rounded-full p-1.5 h-fit border border-input bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50 disabled:pointer-events-none transition-colors",
                className,
            )}
            disabled={isDisabled}
            onClick={(e) => {
